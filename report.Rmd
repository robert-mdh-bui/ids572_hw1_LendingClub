...
As conventional for financial accounting, we have to take into account that the accrual value calculation has to be carried out at the end of the last year of calculation period. Thus assuming that the annuity period starts at the beginning of the first fiscal year, then `t` would equal:
```{r}
t = lastFY - firstFY
```
...
$$A=P(1+r)^t$$
We can derive the interest rate as:
...
Most borrowers have more than 10 years' worth of employment, and the default rate decreases as employment length increases, signifying that borrowers with a longer employment length are less likely to default on their loan. Those for whom employment information is n/a tend to have the highest rate of default.
Grade-wise, the longer the borrowers' employment history, the more likely they are to be assigned a prime A/B grade. N/A cases have the highest proportion of lower (C and below) grades, and tend to borrow less money than those who do provide employment length information.
...
For real returns, LC's APY from lending money increases on the aggregate as the `emp_length` variable increases. Those with missing data (potentially unemployed, or student - both of which are less financially stable groups) nets LC the lowest actual return rate.
...
Income here correlates with default rates pretty well - as we go up the deciles, default rates fall consistently. 
Unsurprisingly, income decile also correlates with credit risk grade - higher income deciles have higher proportions of good grades. The distribution of loan amount stratified by income, however, is interesting: lower income deciles, as expected, apply to borrow less money than their higher-income counterparts, but higher income deciles have more variety in the amount loaned.
...
As expected, higher debt-to-income ratio deciles are correlated with a higher rate of default. This can be a good attribute for predicting default.
